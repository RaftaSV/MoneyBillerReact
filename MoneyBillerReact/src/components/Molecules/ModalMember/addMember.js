...
import {Form, DivLeft, DivRight} from 'components/Molecules/ModalUser/style';
...
const AddUserModal = ({ isOpen, onCancel, onRefresh, isUpdate = false, user = null }) => {
  const [createOrUpdateMember, { loading: loadingAddOrUpdateMember }] = useMutation(isUpdate ? `/Users/${user?.id}` : '/Users', {
  ...
  return (
    <Modal
      ...
      isOpen={isOpen}
      onCancel={onCancel}
      title={isUpdate ? 'Actualizar Socio' : 'Agregar Socio'}
      okText={isUpdate ? 'Actualizar' : 'Guardar'}
      okProps={{
        type: 'submit',
        form: 'form-User',
        loading: loadingAddOrUpdateMember
      }}
    >
      <Form id="form-User" method="POST" onSubmit={onSubmit}>
        ...
        <DivRight>
          ...
          <Input name="company" placeholder="Empresa" type="text" defaultValue={user?.company} required/>
